...
class BiLSTM_Attention(nn.Module):
    ...
    def attention_net(self, lstm_output, final_state):
        ...
        # [batch_size,n_hidden*2,len_seq] * [batch_size,n_step,1] = [batch_size,n_hidden*2,1]
    ...
    def forward(self, X):
        ...
        # hidden_state:[num_layers(=1)*num_direction(=2),batch_size,n_hidden]
        hidden_state = Variable(torch.zeros(1*2,len(X),n_hidden))
        cell_state = Variable(torch.zeros(1*2, len(X), n_hidden))

        # final_hidden,final_cell:[num_layers( = 1) *num_directions( = 2), batch_size, n_hidden]
        # output: [len_seq,batch_size,n_hidden*2]
